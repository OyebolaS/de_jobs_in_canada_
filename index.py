...
def select_and_save(dataengineering_jobs, transformed_job_data):
    ...
    with open(dataengineering_jobs, 'r') as file:
        data = json.load(file).get('data', [])  # Extract 'data' key and handle missing key
...
# Replace 'dataengineering_jobs.json' and 'transformed_job_data.csv' with your file names
...
conn=psycopg2.connect(f"postgresql+psycopg2://{db_user}:{db_password}@{db_host}:5432/{db_database}")
cursor=conn.cursor()
